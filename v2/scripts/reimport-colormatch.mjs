// Align each colour NAME to the WooCommerce gallery image whose real garment
// colour is closest to the name's expected colour. WC has no name↔image link,
// so we: (1) sample every gallery front image's garment colour, (2) for each
// variation name compute an expected colour, (3) greedily match names to the
// nearest unused gallery colour code, (4) upload that code's views to the
// variant. Swatch hex stays name-based (set separately by reset-hex).
...
// ---- expected colour per name (keyword map) ----
const MAP = [
...
for (const wp of products) {
  ...
  for (const name of names) {
    ...
    for (const code of codes) { if (used.has(code) && used.size < codes.length) continue; const d = dist(exp, codeColor[code]); if (d < bestD) { bestD = d; best = code; } }
