// Audit: for every product & variant, compare the DB-derived swatch hex against
// the colour NAME's expected hex. Flag large mismatches (likely wrong WC image
// mapped to the name) and missing front images. Read-only, no writes.
import { admin } from "./checks/_clients.mjs";

// same keyword map as reimport-colormatch.mjs (expected colour per name)
const MAP = [
...
function expected(name){const n=(name||"").toLowerCase();for(const[k,h]of MAP)if(n.includes(k))return h;return null;}
...
const THRESH = 80; // rgb euclidean; >THRESH => likely wrong image mapped to name

const products = await admin.from("products").select("id,name,slug").order("name");
let nVar=0, nNoImg=0, nNoHex=0, nSuspect=0, nUnknown=0;
const suspects=[];

for(const p of products.data){
  const v = await admin.from("variants")
    .select("id,hex,colors(name),variant_images(view)")
    .eq("product_id",p.id).order("sort_order");
  const rows = v.data||[];
  const localSus=[];
  for(const x of rows){
    nVar++;
    const name=x.colors?.name?.trim()||"?";
    const hasFront=(x.variant_images||[]).some(i=>i.view==="front");
    if(!hasFront) nNoImg++;
    if(!x.hex){ nNoHex++; localSus.push(`   ⚠ ${name}: KEIN hex${hasFront?"":" +KEIN Bild"}`); continue; }
    const exp=expected(name);
    if(!exp){ nUnknown++; continue; } // name not in map, can't judge
    const d=dist(x.hex,exp);
    if(d>THRESH){ nSuspect++; localSus.push(`   ✗ ${name.padEnd(20)} swatch ${x.hex} vs erwartet ${exp}  Δ${Math.round(d)}${hasFront?"":" +KEIN Bild"}`); }
  }
  if(localSus.length){ suspects.push(`\n▉ ${p.name} (${rows.length} Var)`); suspects.push(...localSus); }
}

console.log(suspects.join("\n")||"(keine Auffälligkeiten)");
console.log(`\n──────── ZUSAMMENFASSUNG ────────`);
console.log(`Produkte:            ${products.data.length}`);
console.log(`Varianten gesamt:    ${nVar}`);
console.log(`ohne Bild:           ${nNoImg}`);
console.log(`ohne hex:            ${nNoHex}`);
console.log(`Name nicht in Map:   ${nUnknown} (nicht bewertbar)`);
console.log(`Verdächtig (Δ>${THRESH}):  ${nSuspect}`);
console.log(`OK:                  ${nVar-nNoHex-nUnknown-nSuspect}`);
